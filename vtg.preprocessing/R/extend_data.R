...
extend_data <- function(data) {
  ...
  temp.data <- temp.data %>%
    ...
    mutate(siteName = case_when(
      d11_siterare == 1 | d11_sitecomrar == 1 ~ "Nasal cavity and paranasal sinuses",
    ...
    mutate(groups = case_when(
      f32_1_systsetting %in% c(2, 3) & Salivary == 1 & f03_surgintent == 2 & (f54_radiointent == 2 & f55_radiosett %in% c(3, 4)) ~ 2,
